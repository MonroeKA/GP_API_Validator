...
export async function verifyAllGlobalPaymentsSdks() {
    ...
    return Promise.all([
        // Resolve the SDK from the repository-level node_modules installed by
        // npm. Running with the verifier directory as cwd makes ESM package
        // resolution skip that dependency.
        runExternalSdkVerifier(process.execPath, [path.join(nodeDirectory, 'verify.js')], repoRoot, envVars, {
    ...
    for (const code of codesToTest) {
        ...
        try {
            ...
            switch (code) {
                ...
                case '403':
                    // A valid restricted merchant/account cannot be created by the
                    // validator. Fake identifiers return 400 (invalid configuration),
                    // not the documented 403, so reporting that probe as a failure
                    // would conflate a sandbox limitation with documentation drift.
                    ...
                    testResults[code] = {
                        ...
                        matched: null,
                        skipped: true,
                        skipReason: 'A valid restricted resource is required; fake sandbox identifiers return 400 instead of a deterministic 403.',
                        statusText: 'Skipped',
                        responseTime: 0
                    };
                    continue;
            ...
            // The sandbox returns INVALID_TRANSACTION_ACTION for a synthetic
            // missing path on collection endpoints while still returning HTTP
            // 404. Keep the status verification, but do not fail the run on a
            // response-code enum that this deterministic probe cannot control.
            if (code === '404' && statusCodeMatched && !contentCheck.contentValid) {
                testResults[code].matched = null;
                testResults[code].skipped = true;
                testResults[code].skipReason = `Sandbox returned HTTP 404 with ${result.body?.error_code || 'an undocumented error code'} for the synthetic missing-resource probe.`;
            }
